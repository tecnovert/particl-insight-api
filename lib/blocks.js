  ...
  if (blockCached) {
    ...
    self.node.getBlock(hash, function(err, block) {
      ...
      self.node.services.bitcoind.getBlockHeader(hash, function(err, info) {
        ...
        self.node.services.bitcoind.getBlockBalances(hash, function(err, balancesinfo) {
          if (err) {
            return self.common.handleErrors(err, res);
          }
          info.plainbalance = balancesinfo.plain;
          info.blindbalance = balancesinfo.blind;
          info.anonbalance = balancesinfo.anon;
...
BlockController.prototype.transformBlock = function(block, info) {
  ...
  return {
    ...
    plainbalance: info.plainbalance,
    blindbalance: info.blindbalance,
    anonbalance: info.anonbalance
  };
};
